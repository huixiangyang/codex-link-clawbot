...
func (h *Handler) diagnoseNoReply(userID string) noReplyDiagnosis {
	if h.remoteLock != nil && h.remoteLock.IsLocked(userID) {
		return noReplyDiagnosis{
			Conclusion: "codex-link-clawbot 已被远程锁定，消息不会进入 Codex。",
			Evidence:   []string{"当前绑定者处于锁定状态。", "锁定时 codex-link-clawbot 请求队列保持暂停。"},
	...
	if hasRuntime {
		...
		case runtimecontrol.StateDraining:
			return noReplyDiagnosis{
				Conclusion: "codex-link-clawbot 正在排空，已接收的请求不会开始执行。",
		...
		case runtimecontrol.StateStarting:
			return noReplyDiagnosis{Conclusion: "服务仍在启动，Codex 和微信监控尚未全部就绪。", Actions: []string{"等待启动完成。", "在主机执行 codex-link-clawbot status。"}}
		}
		if !snapshot.Codex.Ready {
			return noReplyDiagnosis{
				Conclusion: "Codex 应用服务未就绪，codex-link-clawbot 请求不能启动 Codex 轮次。",
				Evidence:   []string{"运行控制器报告 Codex ready=false。"},
				Actions:    []string{"在主机执行 codex-link-clawbot status。", "检查 Codex 登录后重启服务。"},
			}
		}
		if snapshot.WeChat.Monitors == 0 || snapshot.WeChat.Healthy < snapshot.WeChat.Monitors {
			return noReplyDiagnosis{
				...
				Actions:    []string{"在主机执行 codex-link-clawbot status。", "持续异常时重新登录微信并重启服务。"},
			}
		}
		if snapshot.WeChat.LastSuccessSecondsAgo < 0 || time.Duration(snapshot.WeChat.LastSuccessSecondsAgo)*time.Second > staleWeChatPollAfter {
			...
			return noReplyDiagnosis{
				...
				Actions:    []string{"在主机执行 codex-link-clawbot status。", "检查网络与微信登录状态。"},
			}
		}
	...
	if failure, exists := statefile.LastFailure(); exists && time.Since(failure.At) >= 0 && time.Since(failure.At) <= recentStateFailure {
		return noReplyDiagnosis{
			Conclusion: "最近一次持久化失败，消息或 codex-link-clawbot 请求可能没有安全落盘。",
	...
	if h.tasks == nil {
		return noReplyDiagnosis{Conclusion: "codex-link-clawbot 请求队列未初始化。", Actions: []string{"在主机重启 codex-link-clawbot。"}}
	}
	status := h.tasks.Status(userID)
	if status.Paused {
		return noReplyDiagnosis{
			Conclusion: "当前绑定者的 codex-link-clawbot 请求队列已暂停。",
	...
	for _, task := range tasks {
		...
		case request.StateRunning:
			return noReplyDiagnosis{
				Conclusion: "codex-link-clawbot 已接收请求，Codex 轮次正在执行。",
				Evidence:   []string{"codex-link-clawbot 执行状态：运行中。"},
				Actions:    []string{"发送“执行状态”查看进度。", "不再需要时发送“取消当前执行”。"},
			}
		case request.StateDelivering:
			return noReplyDiagnosis{
				Conclusion: "Codex 已完成，结果正在发送。",
				Evidence:   []string{"codex-link-clawbot 执行状态：发送中。"},
	...
	if status.Queued > 0 {
		...
		if hasRuntime && snapshot.Tasks.Running+snapshot.Tasks.Delivering > 0 {
			evidence = append(evidence, "codex-link-clawbot 全局协调器正在处理另一个请求。")
		}
		return noReplyDiagnosis{
			Conclusion: "请求已进入 codex-link-clawbot 队列，尚未开始 Codex 轮次。",
			Evidence:   evidence,
	...
	for _, task := range tasks {
		if task.State != request.StateFailed && task.State != request.StateInterrupted {
			continue
		}
	...
	evidence := []string{"Codex 已就绪。", "codex-link-clawbot 请求队列未暂停，当前没有等待或执行请求。"}
	...
	return noReplyDiagnosis{
		...
		Evidence:   evidence,
		Actions:    []string{"发送“请求队列”确认是否曾入队。", "仍异常时在主机执行 codex-link-clawbot status。"},
	}
}
...
func (h *Handler) isNoReplyDiagnostic(text string) bool {
	...
	return found && resolved.Definition.ID == control.IntentNoReplyDiagnostic
}

func diagnoseTerminalTask(task request.Task) (string, []string, []string) {
	age := formatUptime(time.Since(time.Unix(task.FinishedAt, 0)))
	switch task.Reason {
	case request.ReasonDeliveryAmbiguous, request.ReasonRestartDelivery:
	...
	case request.ReasonRestartRunning:
...
func stateFailureActions(category statefile.Category) []string {
	...
	case statefile.CategoryPermission:
		return []string{"在主机检查 ~/.codex-link-clawbot 所有者和权限。", "修复后重启服务。"}
	case statefile.CategoryCorrupt, statefile.CategorySchema:
		return []string{"停止服务并检查最近备份清单。", "不要手工覆盖损坏状态。"}
	default:
		return []string{"在主机执行 codex-link-clawbot status。", "检查存储后再重试。"}
	}
}
