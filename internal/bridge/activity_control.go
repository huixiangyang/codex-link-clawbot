...
func (h *Handler) openActivityDetail(userID, id string, pageNumber int) string {
	...
	case request.StateFailed, request.StateInterrupted:
	...
	if task.ThreadID != "" {
		prompt += "\nCodex 线程：" + thread.ShortCode(task.ThreadID)
	}
...
func taskDeliveryStateText(task request.Task) string {
	switch task.State {
	case request.StateQueued, request.StateRunning:
		return "尚未开始"
	case request.StateDelivering:
		return "正在发送"
	case request.StateSucceeded:
		return "已成功发送"
	case request.StateInterrupted:
		if task.Reason == request.ReasonDeliveryAmbiguous || task.Reason == request.ReasonRestartDelivery {
			return "发送结果不确定 · 可人工取回冻结文字"
		}
		return "未发送"
	case request.StateFailed:
		if task.Reason == request.ReasonDeliveryFailed {
			return "微信明确发送失败"
		}
		return "未发送"
	case request.StateCancelled:
...
func taskStateText(state request.State) string {
	switch state {
	case request.StateQueued:
		return "排队中"
	case request.StateRunning:
...
func (h *Handler) requestTaskRetry(userID, taskID string) ActionResult {
	if h.tasks == nil {
		return newActionResult(string(actionTaskRetry), control.DomainQueue, "codex-link-clawbot 请求队列当前不可用。")
	}
	if task, ok := h.tasks.Find(userID, taskID); !ok || task.State != request.StateFailed && task.State != request.StateInterrupted || h.hasFrozenDelivery(task) {
		return newActionResult(string(actionTaskRetry), control.DomainQueue, "这条执行记录已经不能重试。发送“请求队列”刷新。")
	}
	return effectActionResult(string(actionTaskRetry), control.DomainQueue, "正在创建重试请求。", EffectRetryTask, taskID)
}

func (h *Handler) requestFrozenTaskText(userID, taskID string) ActionResult {
	if h.tasks == nil {
		return newActionResult(string(actionTaskFrozenText), control.DomainQueue, "codex-link-clawbot 请求队列当前不可用。")
	}
	task, ok := h.tasks.Find(userID, taskID)
	if !ok || !h.hasFrozenDelivery(task) {
		return newActionResult(string(actionTaskFrozenText), control.DomainQueue, "这条执行记录没有可人工恢复的冻结结果。")
	}
	if _, err := h.tasks.LoadResult(userID, taskID); err != nil {
		return newActionResult(string(actionTaskFrozenText), control.DomainQueue, "冻结结果已过期或损坏，无法取回。")
	}
	return effectActionResult(string(actionTaskFrozenText), control.DomainQueue, "正在取回冻结文字。", EffectFrozenText, taskID)
}

func (h *Handler) hasFrozenDelivery(task request.Task) bool {
	switch task.Reason {
	case request.ReasonRestartDelivery, request.ReasonDeliveryFailed, request.ReasonDeliveryAmbiguous:
		if h.tasks == nil {
			return false
		}
