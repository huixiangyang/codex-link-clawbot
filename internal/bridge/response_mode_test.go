...
func TestResponseModeMenuPersistsVoicePreference(t *testing.T) {
	...
	handler := newControlTestHandler(t)
	handler.preferences = store
	handler.visual = &fakeControlVisualRenderer{}
	handler.voice = NewVoiceBriefing("/usr/bin/ffmpeg", nil)
	...
	if got := store.Get("owner-1").ResponseMode; got != presentation.ResponseVoice {
	...
	if got := reloaded.Get("owner-1").ResponseMode; got != presentation.ResponseVoice {
	...
	if !handled || !strings.Contains(direct.Text, "当前：自适应") || store.Get("owner-1").ResponseMode != presentation.ResponseAdaptive {
...
`)
	...
	handler.voice = NewVoiceBriefing(ffmpeg, []VoiceProviderEntry{{Provider: provider, Timeout: time.Second}})
...
func TestReadingModeForcesShortReplyCard(t *testing.T) {
	...
	if err := store.SetResponseMode("owner-1", presentation.ResponseReading); err != nil {
		t.Fatal(err)
	}
	renderer := &fakeControlVisualRenderer{path: cardPath}
	handler := newBareHandler(nil)
	handler.preferences = store
	handler.visual = renderer
	handler.visualReplyEnabled = false
	handler.visualReplyMinRunes = 5000
	client := ilink.NewClient(&ilink.Credentials{BotToken: "token", ILinkBotID: "bot-1", ILinkUserID: "owner-1", BaseURL: server.URL})
...
func TestVoiceModeFallsBackToCompleteReadingCardBeforeVisibility(t *testing.T) {
	...
	_ = store.SetResponseMode("owner-1", presentation.ResponseVoice)
	renderer := &fakeControlVisualRenderer{path: cardPath}
	handler := newBareHandler(nil)
	handler.preferences = store
	handler.visual = renderer
	handler.voice = NewVoiceBriefing("/usr/bin/ffmpeg", []VoiceProviderEntry{{
