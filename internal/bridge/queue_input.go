...
func prepareQueuedInputWithDownloaders(ctx context.Context, text string, images []*ilink.ImageItem, files []*ilink.FileItem, downloaders inboundDownloaders) (string, []request.InputAttachment, []request.InputAttachment, error) {
	...
	queuedImages := make([]request.InputAttachment, 0, len(images))
	queuedFiles := make([]request.InputAttachment, 0, len(files))
	...
	for index, image := range images {
		...
		queuedImages = append(queuedImages, request.InputAttachment{
	...
	for index, file := range files {
		...
		queuedFiles = append(queuedFiles, request.InputAttachment{
