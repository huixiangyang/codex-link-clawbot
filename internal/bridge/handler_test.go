...
func newControlTestHandler(t *testing.T) *Handler {
	t.Helper()
...
func TestRuntimeCenterShowsBridgeAndCodexIdentity(t *testing.T) {
	...
	for _, want := range []string{
		...
		"模型：使用 Codex 默认配置", "Codex 工作目录：" + handler.projects.Current("owner-1").Root, "Codex PID：4242",
