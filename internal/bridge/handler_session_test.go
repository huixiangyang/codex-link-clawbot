...
func (a *handlerThreadClient) ReviewThread(_ context.Context, threadID, _ string, _ codex.ReviewTarget, _ codex.TurnPhaseHandler) (string, error) {
...
func newSessionHandler(t *testing.T) (*Handler, *handlerThreadClient) {
	...
	handler := newBareHandler(threadAgent)
	attachTestControlState(t, handler)
	attachTestSessionManager(t, handler)
	root := t.TempDir()
	projects, err := workspace.NewManager([]workspace.Definition{{ID: "workspace", Name: "Workspace", Root: root}}, t.TempDir()+"/project-state.json")
	if err != nil {
		t.Fatal(err)
	}
...
func TestGlobalWorkbenchShowsRecentStateAndSwitchesTargetDirectly(t *testing.T) {
	...
	if _, err := handler.sessions.UseGlobalThread(context.Background(), "owner-1", thread.Workspace{ID: "workspace", Name: "Workspace", Root: runtime.cwd}, currentID, runtime); err != nil {
		t.Fatal(err)
	}
...
func TestGlobalWorkbenchPinsOlderCurrentThreadIntoCompactList(t *testing.T) {
	...
	if _, err := handler.sessions.UseGlobalThread(context.Background(), "owner-1", thread.Workspace{ID: "workspace", Name: "Workspace", Root: runtime.cwd}, ids[0], runtime); err != nil {
		t.Fatal(err)
	}
...
func TestCurrentSessionDetailOffersQuickManagement(t *testing.T) {
	...
	detail := controlReply(t, handler, "owner-1", "当前线程")
	for _, want := range []string{"当前线程", "视觉管理", "1  线程关系图", "2  重命名线程", "3  分叉线程", "7  清除线程目标", "10  切换其他线程", "11  归档线程"} {
		if !strings.Contains(detail, want) {
			t.Fatalf("session detail missing %q: %q", want, detail)
		}
	}
	renamePrompt := controlReply(t, handler, "owner-1", "2")
...
func TestThreadRelationsShowNativeOneLevelGraphAndRevalidateSelection(t *testing.T) {
	handler, runtime := newSessionHandler(t)
	...
	childID := fmt.Sprintf("019fcc03-fc8b-7842-a812-%012d", runtime.next)
	...
	reply := controlReply(t, handler, "owner-1", "线程关系图")
	for _, want := range []string{"Codex 线程关系图", "当前目标：关系根", "父线程：无", "直接子线程：1 个可见", "1  直接子线程", "8  刷新关系图", "9  全部线程"} {
		if !strings.Contains(reply, want) {
			t.Fatalf("thread relations missing %q: %q", want, reply)
		}
	}
	state, status, err := handler.controlStates.Load("owner-1")
	if err != nil || status != controlStateActive || state.View != viewSessionRelations || state.Options[0].Value != childID || state.Options[0].Query != "workspace" {
		t.Fatalf("thread relation state = %#v status=%v err=%v", state, status, err)
	}

	child := runtime.threads[childID]
	child.Cwd = t.TempDir()
	runtime.threads[childID] = child
	rejected := controlReply(t, handler, "owner-1", "1")
	if !strings.Contains(rejected, "不在指定受信任工作空间") {
		t.Fatalf("thread relation boundary was not revalidated: %q", rejected)
	}
}
...
func TestFuzzySessionMatchingSupportsSubsequence(t *testing.T) {
	items := []thread.ManagedThread{{Info: codex.ThreadInfo{ID: "thread-1", Name: "微信登录排障"}}}
...
func TestTaskSessionUsesOwnedExplicitThread(t *testing.T) {
	...
	project := handler.projects.Current("owner-1")
	opened, err := handler.sessions.OpenTaskThread(context.Background(), "owner-1", thread.Workspace{
		ID: project.ID, Name: project.Name, Root: project.Root,
	}, "", threadAgent, "检查项目")
	if err != nil {
		t.Fatal(err)
	}
	reply, err := threadAgent.ChatThread(context.Background(), opened.ID, codex.ChatRequest{Text: "检查项目", WorkspaceRoot: project.Root})
