...
func (h *Handler) reviewCurrentThreadPage(ctx context.Context, userID string) controlPage {
	return h.withRuntimeMutationPage(func() controlPage {
		if h.projects == nil || h.sessions == nil {
			return textControlPage("代码审查失败：Codex 工作空间或线程管理器不可用。")
		}
		threadClient, _, err := h.advancedThreadContext()
		if err != nil {
			return textControlPage(err.Error())
		}
		current, err := h.sessions.Current(ctx, userID, threadClient)
		if err != nil {
			return textControlPage(formatSessionError(err))
		}
...
func (h *Handler) reviewFrozenThreadPage(ctx context.Context, userID, projectID, threadID string) controlPage {
	return h.withRuntimeMutationPage(func() controlPage {
		return h.runMobileReview(ctx, userID, projectID, threadID)
	})
}

func (h *Handler) runMobileReview(ctx context.Context, userID, projectID, threadID string) controlPage {
	...
	if !exists {
		return textControlPage("代码审查失败：目标工作空间已不在允许清单中。")
	}
	...
	if err != nil {
		return textControlPage("代码审查失败：" + formatSessionError(err))
	}
	...
	review, err := h.sessions.ReviewProjectThread(
		ctx, userID, projectID, threadID, project.Root, advanced,
		codex.ReviewTarget{Type: "uncommittedChanges"}, nil,
	)
	if err != nil {
		return textControlPage("代码审查失败：" + err.Error())
	}
	review = strings.TrimSpace(review)
	if review == "" {
		return textControlPage("代码审查失败：Codex 没有返回审查结论。")
	}
	...
	model := mobileReviewVisual(packet, options)
	return controlPage{Text: formatMobileReviewPackage(packet, options), visual: &actionVisual{Review: &model}}
}

func mobileReviewVisual(packet mobileReviewPackage, options []controlOption) visual.Review {
	model := visual.Review{
		Verdict: visual.ReviewVerdict(packet.Verdict), Headline: packet.Headline, Summary: packet.Summary,
		Workspace: normalizeReviewRunes(packet.Workspace, 36), Thread: normalizeReviewRunes(packet.Thread, 48),
		Target: packet.Target, Highest: packet.Highest,
		Footer: "回复数字继续；回复“文字版”获取完整审查原文；0 返回 Codex 开发",
	}
	for _, fact := range packet.Facts {
		model.Facts = append(model.Facts, visual.Fact{Label: strings.TrimSuffix(fact.Label, "事实"), Value: fact.Value})
	}
	for _, finding := range packet.Findings {
		model.Findings = append(model.Findings, visual.ReviewFinding{
			Priority: finding.Priority, Title: finding.Title, Detail: finding.Detail, Location: finding.Location,
		})
	}
	for index, option := range options {
		model.Options = append(model.Options, visual.Option{Number: fmt.Sprintf("%d", index+1), Label: option.Label})
	}
	return model
}
...
func (h *Handler) acceptReviewTarget(userID, projectID, threadID string) string {
	...
	return strings.Join([]string{
		"审查结论已接受",
		"工作空间：" + project.Name,
		"目标线程：" + thread.ShortCode(threadID),
