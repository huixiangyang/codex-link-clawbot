...
func (h *Handler) buildGlobalWorkbench(ctx context.Context, userID string) controlPage {
	...
	for _, option := range quickActions {
		label, meta := splitDirectoryLabel(option.Label)
		actions = append(actions, visual.WorkbenchAction{Code: option.Code, Label: label, Meta: meta, Icon: icons[option.Code]})
	}
	model := &visual.Workbench{
		Title: "全局工作台", Subtitle: "从微信统筹 Codex 桌面端、CLI 与远程执行", State: globalState,
		Facts: []visual.Fact{
			{Label: "工作空间", Value: fmt.Sprintf("%d 个", workspaceCount)},
			{Label: "全部线程", Value: fmt.Sprintf("%d 个", totalThreads)},
...
func workbenchRecentThreads(items []thread.GlobalThread) []thread.GlobalThread {
	if len(items) <= workbenchRecentThreadLimit {
		return append([]thread.GlobalThread(nil), items...)
	}
	recent := append([]thread.GlobalThread(nil), items[:workbenchRecentThreadLimit]...)
...
func (h *Handler) buildCommandDirectory(ctx context.Context, userID string) controlPage {
	currentSession := "未选择"
	if h.sessions != nil {
	...
	model := &visual.Directory{
		Title: "Codex 全部功能", Subtitle: "按领域浏览 Codex 与 codex-link-clawbot 控制能力",
		Facts: []visual.Fact{
			{Label: "工作空间", Value: fmt.Sprintf("%d 个 · 当前 %s", workspaceCount, projectName)},
