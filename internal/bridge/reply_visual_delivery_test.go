...
func TestReadingPagesStageAsOneBatchBeforeVisibility(t *testing.T) {
	...
	report := handler.deliverReplyPlan(
		...
		reply, nil, nil, nil, "client", delivery.Source{}, presentation.ResponseReading, presentation.StyleEditorial, "Project",
	)
	...
	if report.Outcome != request.DeliverySucceeded || report.MediaSent != 0 || !report.TextSent {
...
func TestReadingBatchPartialSendNeverDuplicatesFullText(t *testing.T) {
	...
	report := handler.deliverReplyPlan(
		context.Background(), client, ilink.WeixinMessage{FromUserID: "owner", ContextToken: "context"},
		multiPageVisualReply(t), nil, nil, nil, "client", delivery.Source{}, presentation.ResponseReading, presentation.StyleEditorial, "Project",
	)
	...
	if report.Outcome != request.DeliveryAmbiguous || report.MediaSent != 1 || report.TextSent {
