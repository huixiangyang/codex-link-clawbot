...
const (
	...
	viewProjectCenter        controlView = "workspace.center"
	viewProjectResult        controlView = "workspace.result"
	...
	viewSessionRelations     controlView = "thread.relations"
...
type persistedControlReceipt struct {
	...
	Domain    control.Domain `json:"domain"`
...
func (store *ControlStateStore) ReserveReceipt(ownerID, sourceKey, actionID string, domain control.Domain) (bool, error) {
...
func (store *ControlStateStore) ConsumeAndReserve(ownerID, revision, sourceKey, actionID string, domain control.Domain) (consumed, duplicate bool, err error) {
	ownerID = strings.TrimSpace(ownerID)
...
func (store *ControlStateStore) RollbackReservedReceipt(ownerID, sourceKey, actionID string, domain control.Domain) error {
	ownerID = strings.TrimSpace(ownerID)
	sourceKey = strings.TrimSpace(sourceKey)
	actionID = strings.TrimSpace(actionID)
...
func validControlReceiptIdentity(value string, domain control.Domain) bool {
	value = strings.TrimSpace(value)
	if action := controlAction(value); action.valid() {
		return controlActionDomain(action) == domain
	}
	switch control.ID(value) {
	case control.IntentCancel, control.IntentTaskContinue, control.IntentTaskRerun, control.IntentTaskRerunNew,
		control.IntentQueuePause, control.IntentQueueResume, control.IntentQueueClear:
		return domain == control.DomainQueue
	case control.IntentProjectSelect:
		return domain == control.DomainProject
	case control.IntentSessionSelect, control.IntentSessionRestore, control.IntentSessionNew, control.IntentSessionRename, control.IntentSessionArchive,
		control.IntentThreadFork, control.IntentThreadPin, control.IntentThreadCompact, control.IntentThreadGoal,
		control.IntentThreadGoalClear, control.IntentThreadReview, control.IntentTurnSteer:
		return domain == control.DomainSession
	case control.IntentResponseVoice, control.IntentResponseAdaptive, control.IntentResponseReading, control.IntentVisualStyle:
		return domain == control.DomainPreference
	case control.IntentVoiceBriefing:
		return domain == control.DomainQueue
	case control.IntentRemoteLock:
		return domain == control.DomainSecurity
	default:
		return false
	}
}
