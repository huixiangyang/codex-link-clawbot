...
func (h *Handler) executeSessionControlAction(ctx context.Context, userID string, option controlOption) ActionResult {
	...
	case actionThreadRelations:
		return pageActionResult(string(option.Action), control.DomainSession, h.buildCurrentThreadRelations(ctx, userID))
	...
	case actionReviewRerun:
		if h.hasActiveTask(userID) {
			...
			return pageActionResult(string(option.Action), control.DomainSession, h.reviewFrozenThreadPage(ctx, userID, option.Query, option.Value))
		}
...
func (h *Handler) dispatchSessionIntent(ctx context.Context, userID string, resolved control.Resolved, argument string) ActionResult {
	var text string
	switch resolved.Definition.ID {
	case control.IntentSessionCenter:
		text = h.openCodexGlobalOverview(ctx, userID)
	case control.IntentSessionSelect:
		if argument == "" {
	...
	case control.IntentSessionSearch:
		if argument == "" {
			text = h.promptCodexGlobalSearch(userID)
		} else {
			text = h.openCodexGlobalThreadPage(ctx, userID, false, false, argument, 1)
		}
	case control.IntentSessionCurrent:
		text = h.currentSessionDetail(ctx, userID)
	case control.IntentThreadRelations:
		return pageActionResult(string(resolved.Definition.ID), resolved.Definition.Domain, h.buildCurrentThreadRelations(ctx, userID))
	case control.IntentSessionRestore:
		text = h.openSessionPicker(ctx, userID, true, argument)
	case control.IntentSessionNew:
		if argument == "" {
			text = h.promptNewSessionName(userID)
		} else {
			text = h.createSession(ctx, userID, argument)
		}
	case control.IntentSessionRename:
		if argument == "" {
			text = h.promptRenameSession(userID)
		} else {
			text = h.renameSession(ctx, userID, argument)
		}
	case control.IntentSessionArchive:
		text = h.confirmArchiveCurrent(ctx, userID)
	case control.IntentThreadFork:
		text = h.forkCurrentThread(ctx, userID)
	case control.IntentThreadPin:
		text = h.toggleCurrentThreadPin(ctx, userID, true)
	case control.IntentThreadCompact:
		text = h.compactCurrentThread(ctx, userID)
	case control.IntentThreadGoal:
		if argument == "" {
			text = h.promptCurrentThreadGoal(userID)
		} else {
			text = h.setCurrentThreadGoal(ctx, userID, argument)
		}
	case control.IntentThreadGoalClear:
		text = h.clearCurrentThreadGoal(ctx, userID)
	case control.IntentThreadReview:
		return pageActionResult(string(resolved.Definition.ID), resolved.Definition.Domain, h.reviewCurrentThreadPage(ctx, userID))
	case control.IntentThreadDelete:
		text = h.confirmDeleteCurrentThread(ctx, userID)
	case control.IntentThreadModels:
		text = h.openThreadModels(ctx, userID)
	case control.IntentThreadEffort:
		text = h.openThreadEfforts(ctx, userID, "")
	case control.IntentTurnSteer:
