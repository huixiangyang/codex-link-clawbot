...
func TestNumberedMenuContinuesAfterHandlerRestart(t *testing.T) {
	...
	restarted := newBareHandler(newHandlerThreadClient())
	restarted.controlStates = restartedStore
	result, handled := restarted.handleControlInput(context.Background(), "owner-1", "9", false, nextTestControlSource())
	if !handled || result.Domain != control.DomainSystem || !strings.Contains(result.Text, "Codex 全局工作台") {
...
func TestPendingSessionInputContinuesAfterHandlerRestart(t *testing.T) {
	...
	restarted := newBareHandler(runtime)
	restarted.controlStates = restartedStore
	restarted.sessions = first.sessions
...
func TestControlReceiptPreventsDuplicateActionAcrossRestart(t *testing.T) {
	...
	reserved, err := store.ReserveReceipt("owner-1", "account:message:42", string(control.IntentRemoteLock), control.DomainSecurity)
	if err != nil || !reserved {
	...
	if reserved, err = store.ReserveReceipt("owner-1", "account:message:42", string(control.IntentQueuePause), control.DomainQueue); err == nil || reserved {
	...
	if !exists || receipt.ActionID != string(control.IntentRemoteLock) || receipt.Domain != control.DomainSecurity {
...
func TestControlStateConsumeAndReceiptCommitTogether(t *testing.T) {
	...
	consumed, duplicate, err := store.ConsumeAndReserve(
		"owner-1", state.Revision, "account:message:81", string(actionSelectProject), control.DomainProject,
	)
...
func TestControlReceiptRollbackRestoresRevision(t *testing.T) {
	...
	consumed, duplicate, err := store.ConsumeAndReserve(
		"owner-1", state.Revision, "account:message:82", string(actionTaskRerun), control.DomainQueue,
	)
	if err != nil || !consumed || duplicate {
...
func TestReservedControlReceiptRollbackRemovesDirectRerun(t *testing.T) {
	...
	if reserved, err := store.ReserveReceipt(
		"owner-1", "account:message:83", string(control.IntentTaskRerun), control.DomainQueue,
	); err != nil || !reserved {
...
func TestControlStateStoreRejectsRetiredV13(t *testing.T) {
	path := filepath.Join(t.TempDir(), "control-state.json")
	if err := os.WriteFile(path, []byte(`{"version":13,"owners":{},"receipts":{}}`), 0o600); err != nil {
		t.Fatal(err)
	}
	if _, err := NewControlStateStore(path); err == nil || !strings.Contains(err.Error(), "invalid control state schema") {
		t.Fatalf("retired control state error = %v", err)
	}
}
