...
func (h *Handler) latestSuccessfulTask(userID string, requireReusable bool) (request.Task, bool) {
	if h.tasks == nil {
		return request.Task{}, false
	}
	var latest request.Task
	for _, task := range h.tasks.List(userID) {
		if task.State != request.StateSucceeded || task.ThreadID == "" {
			continue
		}
		if requireReusable {
...
func (h *Handler) requestSuccessfulTaskRerun(userID, taskID string, newThread bool) ActionResult {
	if h.tasks == nil {
		return newActionResult(string(actionTaskRerun), control.DomainQueue, "codex-link-clawbot 请求队列当前不可用。")
	}
	task, exists := h.tasks.Find(userID, taskID)
	if !exists || task.State != request.StateSucceeded || task.ThreadID == "" {
		return newActionResult(string(actionTaskRerun), control.DomainQueue, "这条执行记录已经不能继续。发送“请求队列”刷新。")
	}
	prompt, err := h.tasks.LoadReusablePrompt(userID, task.ID)
	if err != nil {
		return newActionResult(string(actionTaskRerun), control.DomainQueue, "原始请求已过期，或请求包含附件，不能再次执行。")
	}
	result := effectActionResult(string(actionTaskRerun), control.DomainQueue, "正在创建后续请求。", EffectEnqueuePrompt, prompt).
