...
func newDiagnosticHandler(t *testing.T) (*Handler, *request.Store, *fixedRuntimeLifecycle) {
	t.Helper()
...
func enqueueDiagnosticTask(t *testing.T, store *request.Store, ownerID, source string) request.Task {
	t.Helper()
	task, duplicate, err := store.Enqueue(request.EnqueueInput{
		SourceMessageKey: source,
		OwnerID:          ownerID,
		ProjectID:        "workspace",
		Summary:          "诊断测试",
		Text:             "执行诊断测试",
		ResponseMode:     presentation.ResponseAdaptive,
		VisualStyle:      presentation.StyleEditorial,
	})
...
func TestNoReplyDiagnosticExplainsAmbiguousDelivery(t *testing.T) {
	...
	if _, err := store.Finish("owner-1", queued.ID, request.StateInterrupted, request.ReasonDeliveryAmbiguous); err != nil {
		t.Fatal(err)
	}
...
func TestLockedOwnerCanRequestNoReplyDiagnostic(t *testing.T) {
	...
	lock, err := access.NewRemoteLock(filepath.Join(t.TempDir(), "remote-lock.json"), "private-code")
	if err != nil {
		t.Fatal(err)
	}
