...
func TestTaskCenterUsesPersistentQueueAndKeepsDetailPage(t *testing.T) {
	...
	for index := 0; index < 8; index++ {
		_, _, err := store.Enqueue(request.EnqueueInput{
			SourceMessageKey: "source-" + string(rune('a'+index)), OwnerID: "owner-1", ProjectID: "project",
			Summary: "检查任务", Text: "执行检查", ResponseMode: presentation.ResponseAdaptive, VisualStyle: presentation.StyleEditorial,
		})
		if err != nil {
			t.Fatal(err)
		}
...
func TestTaskCenterOnlyOffersFrozenRecoveryWhenResultExists(t *testing.T) {
	...
	handler.tasks = store
	enqueue := func(source string) request.Task {
		t.Helper()
		_, _, err := store.Enqueue(request.EnqueueInput{
	...
	if _, err := store.Finish("owner-1", withoutResult.ID, request.StateFailed, request.ReasonDeliveryFailed); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := store.FreezeResult("owner-1", withResult.ID, request.FreezeResultInput{Reply: "冻结回答"}); err != nil {
		t.Fatal(err)
	}
	if _, err := store.BeginDelivery("owner-1", withResult.ID); err != nil {
		t.Fatal(err)
	}
	if _, err := store.Finish("owner-1", withResult.ID, request.StateFailed, request.ReasonDeliveryFailed); err != nil {
		t.Fatal(err)
	}
