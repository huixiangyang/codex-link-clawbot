...
func (h *Handler) handleCodexSlashCommand(ctx context.Context, userID, text, sourceKey string, hasAttachments bool) (ActionResult, bool) {
	...
	if !exists {
		return newActionResult("codex.slash.unknown", control.DomainSession, "没有这个可用的 Codex 斜杠命令。发送 / 查看 codex-link-clawbot 可直接执行的命令。"), true
	}
	if hasAttachments {
		return newActionResult("codex.slash."+command.Name, control.DomainSession, "斜杠命令不接收微信附件。请先执行命令，再单独发送图片或文件。"), true
	}
	if command.Mutating {
		if h.hasActiveTask(userID) {
			return newActionResult("codex.slash."+command.Name, control.DomainSession, mutationBusyText()), true
		}
		reserved, result := h.reserveControlReceipt(userID, sourceKey, string(actionCodexSlashCommand), control.DomainSession)
		if !reserved {
			return result, true
		}
	}
	if command.Name == "review" {
		return pageActionResult("codex.slash."+command.Name, control.DomainSession, h.reviewCurrentThreadPage(ctx, userID)), true
	}
	textResult := h.executeCodexSlashCommand(ctx, userID, command, argument, "", 0)
	return newActionResult("codex.slash."+command.Name, control.DomainSession, textResult), true
}
