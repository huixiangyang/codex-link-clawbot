...
type Manager struct {
	store            *Store
	now              func() time.Time
	resolveWorkspace func(ownerID string) Workspace
}

func NewManager(path string, resolveWorkspace func(ownerID string) Workspace) (*Manager, error) {
...
func (m *Manager) currentWorkspace(ownerID string) Workspace {
	workspace := m.resolveWorkspace(strings.TrimSpace(ownerID))
...
func (m *Manager) currentProject(ownerID string) string {
	projectID := m.currentWorkspace(ownerID).ID
	if projectID == "" {
...
func (m *Manager) New(ctx context.Context, ownerID string, client codex.ThreadClient, name string) (codex.ThreadInfo, error) {
	...
	if workspace.ID == "" || workspace.Root == "" {
		return codex.ThreadInfo{}, fmt.Errorf("current workspace is unavailable")
	}
	thread, err := client.StartThread(ctx, workspace.Root)
	if err != nil {
...
func (m *Manager) SteerCurrent(ctx context.Context, ownerID string, advanced codex.AdvancedThreadClient, request codex.ChatRequest) error {
	...
	request.WorkspaceRoot = m.currentWorkspace(ownerID).Root
	return advanced.SteerThread(ctx, threadID, request)
}

func (m *Manager) ReviewCurrent(ctx context.Context, ownerID string, advanced codex.AdvancedThreadClient, target codex.ReviewTarget, progress codex.TurnPhaseHandler) (string, error) {
	threadID, ok := m.store.ActiveForProject(ownerID, m.currentProject(ownerID))
	if !ok {
		return "", ErrNoActive
	}
	return advanced.ReviewThread(ctx, threadID, m.currentWorkspace(ownerID).Root, target, progress)
}
...
func (m *Manager) ReviewProjectThread(
	...
	progress codex.TurnPhaseHandler,
) (string, error) {
	ownerID = strings.TrimSpace(ownerID)
	...
	return advanced.ReviewThread(ctx, threadID, workspaceRoot, target, progress)
}
...
func (m *Manager) Archive(ctx context.Context, ownerID string, client codex.ThreadClient, reference string) (string, error) {
	...
	if strings.TrimSpace(reference) == "" {
		var ok bool
		reference, ok = m.store.ActiveForProject(ownerID, workspace.ID)
		if !ok {
			return "", ErrNoActive
		}
	}
	...
	if activeID == record.ID {
		for _, candidate := range m.store.RecordsForProject(ownerID, projectID, false) {
			...
			if _, readErr := client.ResumeThread(ctx, candidate.ID, workspace.Root); readErr == nil {
