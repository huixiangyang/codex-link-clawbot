...
func (a *Client) ReadThreadVerificationFacts(ctx context.Context, threadID string) (codex.ThreadVerificationFacts, error) {
	...
	if err := json.Unmarshal(result, &response); err != nil {
		return codex.ThreadVerificationFacts{}, fmt.Errorf("parse thread verification facts: %w", err)
	}
	if response.Thread.ID == "" {
		return codex.ThreadVerificationFacts{}, fmt.Errorf("thread/read returned empty thread id")
	}
	for index := len(response.Thread.Turns) - 1; index >= 0; index-- {
		turn := response.Thread.Turns[index]
		facts := codex.ThreadVerificationFacts{Available: true, TurnID: turn.ID}
		if turn.CompletedAt != nil {
			facts.CompletedAt = *turn.CompletedAt
		}
		kindSet := make(map[codex.VerificationKind]bool)
		...
		facts.Kinds = make([]codex.VerificationKind, 0, len(kindSet))
	...
	return codex.ThreadVerificationFacts{Available: true}, nil
}

func verificationKinds(command string) []codex.VerificationKind {
	command = strings.ToLower(strings.ReplaceAll(command, "\r\n", "\n"))
	seen := make(map[codex.VerificationKind]bool)
	for _, candidate := range verificationCommandPatterns {
		if candidate.Pattern.MatchString(command) {
			seen[candidate.Kind] = true
		}
	}
	kinds := make([]codex.VerificationKind, 0, len(seen))
	for kind := range seen {
