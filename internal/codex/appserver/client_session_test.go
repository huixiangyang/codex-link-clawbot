...
var _ codex.AdvancedThreadClient = (*Client)(nil)
var _ codex.CapabilityClient = (*Client)(nil)
var _ codex.TurnProgressClient = (*Client)(nil)
var _ codex.GlobalControlClient = (*Client)(nil)
...
func TestCodexSteersAndReviewsActiveTurn(t *testing.T) {
	...
	a = newCodexSessionTestAgent(func(_ context.Context, method string, params interface{}) (json.RawMessage, error) {
		...
		case "review/start":
			got := params.(map[string]interface{})
			if got["threadId"] != threadID || got["delivery"] != "inline" || got["target"].(codex.ReviewTarget).Type != "uncommittedChanges" {
				t.Fatalf("review/start params = %#v", got)
			}
			a.turnCh[threadID] <- &codexTurnEvent{Kind: "message_completed", TurnID: "turn-review", ItemID: "review-message", Text: "发现一处风险"}
			a.turnCh[threadID] <- &codexTurnEvent{Kind: "terminal", TurnID: "turn-review", Phase: string(codex.TurnPhaseCompleted)}
	...
	if err := a.SteerThread(context.Background(), threadID, codex.ChatRequest{Text: "先修复失败测试"}); err != nil {
		t.Fatal(err)
	}
	delete(a.activeTurns, threadID)
	review, err := a.ReviewThread(context.Background(), threadID, "/workspace", codex.ReviewTarget{Type: "uncommittedChanges"}, nil)
