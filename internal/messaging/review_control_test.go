...
func TestBuildMobileReviewPackageExtractsPriorityAndRedactsPrivatePath(t *testing.T) {
	raw := strings.Join([]string{
		"## Findings",
		"- [P1] Do not lose the delivery receipt — /root/CODES/codex-link-clawbot/internal/messaging/handler.go:221",
		"  Returning early leaves /root/.codex-link-clawbot/private/state.json stale and retries the visible result.",
		"- [P2] Preserve the target thread — internal/session/manager.go:407",
	...
	if packet.Findings[0].Location != "messaging/handler.go:221" || strings.Contains(packet.Findings[0].Detail, "/root/") {
		t.Fatalf("private path was not reduced: %#v", packet.Findings[0])
	}
	if packet.Findings[1].Location != "session/manager.go:407" {
...
func TestMobileReviewActionsFreezeProjectAndThread(t *testing.T) {
	...
	runtime.reviewText = "- [P1] 修复竞态 — /root/CODES/codex-link-clawbot/internal/messaging/control.go:88\n  同一菜单不能执行两次。"
	...
	for _, want := range []string{"Codex 移动审查", "发现 1 项需要判断", "验证事实：2 项 · 2 通过 · 测试", "[P1] 修复竞态", "位置：messaging/control.go:88"} {
	...
	if !strings.Contains(accepted, "审查结论已接受") || !strings.Contains(accepted, session.ShortCode(threadID)) || strings.Contains(accepted, "已提交") {
...
func TestReviewControlFromTextBuildsDedicatedVisual(t *testing.T) {
	reply := strings.Join([]string{
		"Codex 移动审查", "结论：发现 1 项需要判断", "工作空间：codex-link-clawbot", "目标线程：移动审查",
		"审查范围：未提交改动", "审查状态：attention", "最高优先级：P1", "摘要：优先处理高等级问题。", "",
		"可验证事实", "变更事实：12 个文件 · +180 / −42", "验证事实：3 项 · 3 通过 · 测试/检查", "交付事实：1 项可再次发送", "",
		"重点问题", "[P1] 修复投递竞态", "位置：messaging/handler.go:221", "避免重复发送。", "",
		"1  继续修复 · 当前线程", "2  接受结论 · 结束审查", "3  重新审查 · /review", "",
		"回复数字继续；回复“文字版”获取完整审查原文；0 返回 Codex 开发。",
	}, "\n")
	review, ok := reviewControlFromText(reply)
	if !ok || review.Verdict != visual.ReviewVerdictAttention || review.Highest != "P1" || len(review.Facts) != 3 || len(review.Findings) != 1 || len(review.Options) != 3 {
		t.Fatalf("review = %#v ok=%v", review, ok)
	}
	if review.Findings[0].Location != "messaging/handler.go:221" || review.Options[0].Number != "1" {
