...
func (h *Handler) completePendingNotices(ownerID string, notices []PendingNotice) error {
...
func formatPendingNoticeDigest(notices []PendingNotice) string {
	...
	for index, notice := range notices {
		lines = append(lines, "", fmt.Sprintf("[%d] %s", index+1, notice.Title), truncateRunes(strings.TrimSpace(notice.Body), 1200))
	}
