...
func (h *Handler) openDeliveryPage(userID string, pageNumber int) string {
	...
	for _, record := range records[start:end] {
		...
		if h.deliveries.Availability(record) != DeliveryAvailable {
			label += " · 已失效"
		}
...
func (h *Handler) openDeliveryDetail(userID, id string, pageNumber int) string {
	...
	prompt := strings.Join([]string{
		...
		"Codex 线程：" + session.ShortCode(record.ThreadID),
...
func (h *Handler) resendDelivery(userID, id string, pageNumber int) ActionResult {
	if h.deliveries == nil {
		return newActionResult(string(actionResendDelivery), DomainDelivery, "交付箱当前不可用。")
	}
	record, availability, exists := h.deliveries.Verify(userID, id)
	if !exists {
		return newActionResult(string(actionResendDelivery), DomainDelivery, "交付物已经不存在。")
	}
	if availability != DeliveryAvailable {
		return newActionResult(string(actionResendDelivery), DomainDelivery, "交付物私有副本已失效，不能再次发送，也不会自动重跑原请求。")
	}
	...
	if !h.storeChoice(userID, viewDeliveryResult, options, actionDeliveryBox) {
		return newActionResult(string(actionResendDelivery), DomainDelivery, controlStateFailureResult().Text)
	}
	return effectActionResult(string(actionResendDelivery), DomainDelivery, prompt+"\n\n回复数字继续，0 返回。", EffectSendMedia, record.FilePath)
}
