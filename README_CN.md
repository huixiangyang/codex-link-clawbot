...
Codex 与 WeClaw 是两层：Codex 拥有线程、轮次、模型、审查、技能和外部工具；WeClaw 拥有微信接入、项目入口白名单、请求队列、回复呈现、模板、自动化和运维。发送 `/` 得到的 44 项操作总览会在每个分区明确标注归属。
...
发送 `/` 打开一张完整操作总览。`1`、`2` 展示 Codex 线程和执行能力；`3`–`6` 展示 WeClaw 请求队列、回复呈现、内容自动化和运行安全。总览在 30 分钟内有效，可直接回复 `11` 新建 Codex 线程、`22` 选择模型、`24` 审查改动、`31` 查看 WeClaw 执行状态、`48` 切换简洁风格。
