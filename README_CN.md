...
Codex 与 codex-link-clawbot 是两层：Codex 拥有全局线程、轮次、账号、模型、审查、技能和外部工具；codex-link-clawbot 提供微信接入、工作空间白名单、目标线程焦点、请求队列、回复呈现、交付箱和运维。发送“菜单”得到全局工作台，直接查看最近线程、全局遥测和执行状态；图片、文字降级和持久控制状态统一使用稳定数字编号。图片保留 `/command`，仅用于观看和对照 Codex 功能；微信端不接受斜杠输入。

发送“代码审查”或在菜单回复 `32` 会在原线程调用 Codex 原生审查器，并返回专用移动审查卡。卡片除结论、最高优先级、最多三项重点和脱敏位置外，还展示不含文件名、命令和终端输出的变更、验证与交付事实；可继续修复、接受结论或冻结目标后重新审查，30 分钟内回复“文字版”可取回完整审查原文。
...
配置文件位于 `~/.codex-link-clawbot/config.json`，当前结构版本为 6，短期控制状态版本为 15，顶层明确分为 `codex` 与 `codex-link-clawbot`。长任务进度只跟随 App Server 的结构化轮次与工作项通知，不再定时重复“仍在执行”。至少配置一个 Codex 可以进入的工作空间绝对路径；执行 `codex-link-clawbot config` 可查看脱敏后的有效配置。完整字段、视觉、Piper 和 MiMo 配置见 [配置与设置](docs/guides/configuration.md)。
...
发送“菜单”打开 1080×780 Codex 全局工作台：首页左侧展示最近四个线程，每项明确显示项目名称、线程文件目录、状态和活动时间；右上展示工作空间、全部线程、运行中和微信队列遥测，下方直接展示 15 个稳定数字动作。`1`–`4` 接管对应最近线程，`5` 打开全部线程，`6` 新建线程，`7` 查看执行与队列，`8` 打开工作空间，`9` 刷新；`11`–`43` 的稳定能力从同一张图片直接回复。首页状态有效期为 5 分钟，不需要输入英文命令。
...
微信控制面只登记 17 个能够真实执行的 Codex 原生或适配操作，并通过 `25` 下的数字目录进入。图片在每个数字动作旁保留对应 `/command` 作为 Codex 功能说明，用户仍只回复数字。全局线程搜索、账号额度、技能和 MCP 汇总作用于所有受信任工作空间；线程状态、改名、分叉、压缩、目标和模型修改只作用于当前目标线程。CLI、TUI、Windows 与实验协议专属能力不进入微信菜单。任何斜杠前缀输入都会被明确拒绝，也不会进入 Codex 提示词。
