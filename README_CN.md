...
公开入口只有一个 `/`。主菜单始终最多四项：运行中优先显示当前任务，存在等待项时优先显示任务中心；空闲时根据最近结果和当前项目快捷任务动态推荐，再从项目、会话和任务中心补齐，最后固定保留“更多功能”。会话、任务、快捷任务、自动化、素材和交付列表每页展示 6 条，支持数字与“下一页/上一页”。菜单 revision、页码和待输入状态会跨进程重启保留，10 分钟后自动失效；失效后的数字、返回和翻页会明确拒绝，绝不会进入 Codex。
...
“快捷任务”进入当前项目的持久工作流中心；即使列表为空也能直接创建。列表先进入脱敏详情，再选择运行、重命名、编辑或二次确认删除，不会因误触列表而立即执行。发送“新建快捷任务”后按 `名称：…`、`内容：…` 发送一条消息，用 `「分支」` 这类可读标记声明参数。运行时微信逐项询问，十分钟内重启仍从当前项继续；重复消息不会推进两次，最终入队失败会恢复最后一项。提示模板和参数值不写入通用菜单状态，任务固定进入该工作流所属项目。
...
主控制台始终显示当前 WeClaw 版本与项目。“运行中心”展示启动时长、主动发送是否启用、Codex 协议、模型、项目目录、进程号，以及 App Server 推送的主/次额度使用率。选择“为什么没回复”或直接发送同名短语，会读取锁定、运行状态、Codex、微信轮询、持久化、任务队列和最近投递的确定性信号；没有证据时明确说明无法断言，不会调用 Codex 猜测原因，也不会展示账号、正文、路径或原始错误。
...
成功纯文字任务的详情还会提供“继续这个会话”“再次执行”“在新会话执行”和“保存为快捷任务”。继续动作会切回任务冻结的项目与会话；两种再次执行分别固定原线程或显式要求 Coordinator 新建线程，不读取当时的界面选择。为支持这组操作，成功后完整请求、令牌、回答与交付目录仍立即删除，只在私有任务目录保留最长 24 小时的严格 `reusable.json`，内容只有原始文字。包含图片或文件的成功任务不保留复用副本；保存工作流也不会复制回答、附件信息或命令输出。
