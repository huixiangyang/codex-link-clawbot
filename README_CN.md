...
Codex 与 codex-link-clawbot 是两层：Codex 拥有全局线程、轮次、账号、模型、审查、技能和外部工具；codex-link-clawbot 提供微信接入、工作空间白名单、目标线程焦点、请求队列、回复呈现、交付箱和运维。发送“菜单”得到全局工作台，直接查看最近线程、全局遥测和执行状态；图片、文字降级和持久控制状态统一使用稳定数字编号，微信端不接受斜杠命令。
...
微信控制面只登记 17 个能够真实执行的 Codex 原生或适配操作，并通过 `25` 下的数字目录进入。全局线程搜索、账号额度、技能和 MCP 汇总作用于所有受信任工作空间；线程状态、改名、分叉、压缩、目标和模型修改只作用于当前目标线程。CLI、TUI、Windows 与实验协议专属能力不进入微信菜单。任何斜杠前缀输入都会被明确拒绝，也不会进入 Codex 提示词。
