...
发送“菜单”打开 1080×780 Codex 全局工作台：首页左侧展示最近四个线程，右上展示工作空间、全部线程、运行中和微信队列遥测，下方直接展示 15 个稳定数字动作。`1`–`4` 接管对应最近线程，`5` 打开全部线程，`6` 新建线程，`7` 查看执行与队列，`8` 打开工作空间，`9` 刷新；`11`–`43` 的稳定能力从同一张图片直接回复。首页状态有效期为 5 分钟，不需要输入英文命令。
