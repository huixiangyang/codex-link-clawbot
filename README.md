...
Codex owns the global thread catalog, turns, account, models, review, skills, and external tools. codex-link-clawbot adds the WeChat transport, workspace allowlist, target-thread focus, request queue, presentation, deliveries, and operations. Send `菜单` to open the home card. Its image, text fallback, and persisted control state share one stable numeric catalog; WeChat slash commands are not accepted.
