...
func TestManagerRejectsMissingRootAndUnknownProject(t *testing.T) {
	_, err := NewManager([]config.ProjectConfig{{ID: "missing", Name: "Missing", Root: filepath.Join(t.TempDir(), "absent")}}, filepath.Join(t.TempDir(), "state.json"))
	if err == nil {
		t.Fatal("missing root should be rejected")
	}
	manager, err := NewManager([]config.ProjectConfig{{ID: "valid", Name: "Valid", Root: t.TempDir()}}, filepath.Join(t.TempDir(), "state.json"))
	if err != nil {
		t.Fatal(err)
	}
