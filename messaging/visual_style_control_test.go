...
func TestVisualStyleMenuSwitchesPersistsAndIsolatesOwners(t *testing.T) {
	...
	for _, want := range []string{"当前：构筑", "1  刊物", "2  构筑", "3  黑标", "当前"} {
		if !strings.Contains(menu, want) {
			t.Fatalf("style menu missing %q: %q", want, menu)
		}
	}

	switched, handled := handler.handleControlInput(context.Background(), "owner-1", "1", false)
	if !handled || !strings.Contains(switched, "视觉风格已切换") || !strings.Contains(switched, "当前：刊物") {
		t.Fatalf("style switch = %q, handled=%v", switched, handled)
	}
	if got := store.Get("owner-1"); got != visual.StyleEditorial {
	...
	direct, handled := handler.handleControlInput(context.Background(), "owner-1", "切换风格 黑标", false)
	if !handled || !strings.Contains(direct, "当前：黑标") || store.Get("owner-1") != visual.StyleNoir {
	...
	if got := reloaded.Get("owner-1"); got != visual.StyleNoir {
		t.Fatalf("reloaded style = %q", got)
	}

	invalid, handled := handler.handleControlInput(context.Background(), "owner-1", "视觉风格 霓虹", false)
	if !handled || !strings.Contains(invalid, "没有这个视觉风格") || store.Get("owner-1") != visual.StyleNoir {
