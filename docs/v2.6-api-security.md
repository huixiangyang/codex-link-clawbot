...
v2.6 删除了共享 TCP 服务中的无认证发送、健康和管理路由。默认配置不会创建任何 TCP 监听；健康、排空、恢复和部署通知固定通过 `~/.weclaw/control.sock`。主动发送只有在配置显式启用后才启动独立 TCP 服务。
...
weclaw status
go test ./...
go test -race ./api ./config ./cmd ./messaging
go vet ./...
```
