...
WeClaw 是个人微信到本机 Codex App Server 的单一桥接器。它不提供通用 Agent 接口、多模型路由、远程 Shell、任意工作目录或旧协议回退。所有实现包都位于 `internal/`，仓库没有承诺稳定性的公共 Go API。
...
| `internal/reporting` | 确定性 Git、systemd、HTTP 检查与通知调度 |
| `internal/api` | 可选主动发送 HTTP 边界与本机管理协议适配 |
...
cmd/weclaw
  → cli
     → api / reporting / messaging
        → taskqueue / session / workflow / project / preference
...
- 运行健康、部署与恢复只使用当前用户私有 Unix socket；主动 TCP 发送默认不存在。
