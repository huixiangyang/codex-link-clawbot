...
- 发送“视觉风格”在刊物、构筑和黑标三套完整模板间切换；选择按绑定者隔离并同时作用于控制卡和长回复。
...
`~/.weclaw/task-history.json` 采用严格 v1 JSON、原子替换和 `0600` 文件权限，父目录保持 `0700`。记录只包含用户输入第一行的截断摘要、图片/文件数量和时间状态，不保存完整问题、回答、终端输出、文件名、附件路径或 thread ID；读取始终按绑定者隔离。
...
- 三套视觉模板按服务本地时区固定切换明暗模式：07:00–18:59 为明亮主题，19:00–06:59 为深色主题；状态与短操作自动采用高密度多列布局，阅读模式每页使用 32 个视觉容量单位。
- `~/.weclaw/visual-styles.json` 使用严格 v1 JSON、原子替换和 `0600` 权限保存每个绑定者的模板 ID，父目录保持 `0700`。
