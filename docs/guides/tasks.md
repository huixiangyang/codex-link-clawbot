...
发送 `/` 会得到固定编号的操作总览，任务区始终使用 `31`–`35`：查看当前任务、最近结果、暂停/继续队列、取消当前任务和清空等待任务。运行状态只改变标签与可用性提示，不改变编号。也可以直接说“任务中心”“任务队列”“暂停队列”“继续队列”或“清空队列”。
...
本机管理面的健康动作返回 `starting`、`ready`、`draining`、`stopping` 或 `degraded`，以及实际版本、Codex 就绪、微信监控、任务数量和 `drain_complete`。健康、排空、恢复及部署通知只通过当前用户拥有且权限为 `0600` 的 `~/.weclaw/control.sock` 提供；TCP 不再暴露健康或 `/admin/*` 路由。
...
运行中心提供“为什么没回复”，也可以直接发送同名短语。诊断不调用 Codex，不依赖持久菜单或视觉渲染，按远程锁定、运行状态、Codex 就绪、微信监控、最近成功轮询、待提交批次、最近持久化错误、队列状态和最近投递结果的固定顺序返回结论。

当前入站消息会暂时让同步批次处于待提交状态，因此只有最老批次超过 30 秒才判定为卡住。登录过期或微信业务错误不会刷新成功轮询时间。回复最多包含三条确定性依据和三个下一步操作；没有证据时明确说明无法断言，不展示绑定者、正文、附件名、路径、URL、令牌、原始错误或命令输出。
