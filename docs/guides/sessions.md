...
单独发送 `/` 打开视觉操作总览。回复 `1` 进入会话中心，或使用 `11`–`17` 直接新建、重命名、切换、搜索、查看、归档和恢复；需要详情、分页、输入或确认时再进入十分钟子流程。先用 `2` 或 `21` 选择受信任项目，任意微信文本都不能提供本机路径；目录只来自 `projects` 白名单。
...
- 菜单状态按微信用户隔离；操作总览 30 分钟失效，领域子流程十分钟失效。有效编号选择通过原子比较删除，重复到达不会执行两次。任何普通文字和“文字版”取回都会清理旧菜单状态。
...
```bash
go test ./...
go test -race ./internal/codex ./internal/messaging ./internal/session
go vet ./...
```
