...
画布最多展示一个父线程和按标题稳定排序的五个直接子线程，超出部分显示明确截断数量。父线程已删除、归档或位于白名单外时只显示“不可用”，不会从本地索引补造节点。父节点和子节点使用短期编号，回复编号时会再次执行 `thread/read` 与真实路径校验，然后才接管目标；关系状态有效期为十分钟。`8` 刷新本图，`9` 返回全局线程，`0` 返回当前线程。

## 全局工作台

发送“菜单”时，codex-link-clawbot 直接读取全局 `thread/list`，按最近活动时间展示最多四项；标题、匹配后的项目名称、线程真实 `cwd`、状态和相对时间都来自本次快照，不写入菜单文件。长目录只在展示层保留首尾并压缩中段，接管校验仍使用完整路径。当前目标固定在顶部，并在最近列表自然命中时追加标记。回复 `1`–`4` 会消费本版快照、重新读取线程并复核工作空间，然后才恢复并接管目标。
...
以下编号不再要求先进入二级目录，发送“菜单”后可直接回复：

| 编号 | 中文操作 | 命令或范围 |
| --- | --- | --- |
| `11` | 全局总览 | 工作空间、活动、运行、加载、归档与目标线程 |
| `12` | 全局线程 | 中心内包含运行中、全部、搜索与归档 |
| `13` | 账号与额度 | Codex 账号、计划与额度快照 |
| `21` | 工作空间 | 选择默认执行目录 |
| `22` | 目标线程 | 状态与管理入口 |
| `23` | 模型与权限 | 模型、推理强度和执行权限 |
| `24` | 技能与工具 | 技能与 MCP 摘要 |
| `25` | Codex 操作 | 17 个原生或适配操作的数字目录 |
| `31` | 新建工作 | 进入线程名称输入态 |
| `32` | 审查改动 | 调用原生审查器 |
...
- 全局操作：线程目录、账号额度、技能和 MCP 汇总。
- 目标线程操作：状态、改名、归档、删除、分叉、压缩、目标、审查与模型/推理强度修改。
- 新工作操作：新建线程和清屏新建的微信适配动作。
- 客户端专属能力：不进入微信首页、数字操作目录或文字降级菜单，也不转成普通提示词。
...
编号 `32` 或自然语言“代码审查”默认以 inline 方式审查当前目标线程的未提交改动。结果进入专用移动审查卡；“继续修复”和“重新审查”都保存审查时的工作空间 ID 与线程 ID，不会因用户随后切换界面焦点而漂移。“接受结论”只结束本次移动审查交互，不执行提交、推送或部署。
