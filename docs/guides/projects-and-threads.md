...
“Codex 能力”独立展示当前工作入口名称，以及 Codex 返回的启用技能和外部工具连接状态；它不负责切换 WeClaw 项目入口：
...
提示词模板是 WeClaw 的可靠复用功能，不冒充 Codex 技能。项目技能始终来自 Codex 自己的发现结果。“WeClaw 项目入口”页面只展示白名单目录与检查边界，不再混入技能、模型、审查或线程操作。
