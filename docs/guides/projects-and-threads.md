...
发送“菜单”时，codex-link-clawbot 直接读取全局 `thread/list`，按最近活动时间展示最多四项；标题、工作空间、状态和相对时间都来自本次快照，不写入菜单文件。当前目标固定在顶部，并在最近列表自然命中时追加标记。回复 `1`–`4` 会消费本版快照、重新读取线程并复核工作空间，然后才恢复并接管目标。
