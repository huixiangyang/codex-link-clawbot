...
1. 发送“菜单”，确认只收到一张 1080×780 的艺术画布；最近线程与 `5`–`9` 收在顶部左侧，右上单行显示工作空间、全部线程、运行中和微信队列，线程拓扑位于遥测带下方，所有顶部内容不得超过 390 像素；其余区域以三个等宽列面板完整显示 15 个稳定数字动作，对应 `/command` 以次要文字保留并标明仅作功能对照，不得出现 CLI、TUI、Windows、实验协议专属项。当前线程只用柔和选中态和“当前”标记识别，不得出现独立统计卡、当前目标面板或表格边框。
