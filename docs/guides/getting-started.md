...
1. 发送 `/`，确认只收到一张操作总览图片；图片应包含六个领域、六项状态和稳定编号，30 分钟内回复 `11` 可直接进入新建线程。
2. 发送一条普通问题，确认进入 WeClaw 持久请求队列，并在启动 Codex 轮次后收到回复。
3. 回复 `1` 打开线程管理，确认当前项目与线程可见；重新发送 `/` 后回复 `12` 查看当前线程，再选择“重命名线程”。
