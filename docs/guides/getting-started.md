...
1. 发送 `/`，确认只收到一张 1080×780 的艺术画布；最近线程与 `5`–`9` 收在顶部左侧，右上单行显示工作空间、全部线程、运行中和微信队列，线程拓扑位于遥测带下方，所有顶部内容不得超过 390 像素；其余区域以三个等宽列面板完整显示 17 个微信可用 Codex 命令，不得出现 CLI、TUI、Windows 或实验协议专属项。当前线程只用柔和选中态和“当前”标记识别，不得出现独立统计卡、当前目标面板或表格边框。
2. 发送一条普通问题，确认进入 codex-link-clawbot 持久请求队列，并在启动 Codex 轮次后收到回复。
3. 回复 `5` 打开全局线程，确认当前线程可见；重新发送 `/` 后直接回复 `22`，或发送 `/status` 查看目标线程；回复 `8` 应直接进入工作空间，最后用 `/rename 新名称` 重命名。
