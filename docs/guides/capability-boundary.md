...
| 分区 | 归属 |
| --- | --- |
| `1` Codex · 全局 | 总览、统一全局线程、账号与额度 |
| `2` Codex · 工作空间 | 工作空间、目标线程、模型权限、技能工具、Codex 数字操作 |
| `3` Codex · 执行 | 新建、审查、请求队列和取消 |
| `4` codex-link-clawbot · 远程 | 最近结果与交付箱、系统健康与诊断、呈现与安全 |

内部持久标识也遵守同一边界：真正的 Codex 轮次使用 `turn.*`，codex-link-clawbot 队列使用 `queue.*`，Codex 线程使用 `thread.*`。微信数字动作由固定目录解析；客户端专属项不登记，斜杠输入不会下沉成普通提示词。旧的队列 `turn.*` 控制状态不会兼容读取。
