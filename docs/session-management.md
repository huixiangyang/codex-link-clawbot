...
单独发送 `/` 打开视觉主菜单，再用数字进入会话中心。主菜单、会话列表、详情、状态和确认结果由固定 HTML 模板渲染为手机纵向 PNG；需要选择或输入时同时发送一条简短文字提示。会话中心展示当前会话、可用数量和归档数量，并提供当前详情、会话列表、搜索、新建、重命名、归档和恢复。普通列表点选只进入会话详情，展示状态、目录、时间和经过内部标记清洗的提示摘要，再由用户明确选择切换或归档；从详情返回保留原搜索词和页码。当前会话详情继续提供重命名、切换和归档快捷入口；名称输入、搜索输入、归档确认和任务取消确认复用同一个十分钟短期状态机。回复 `0` 返回，回复“取消”退出当前操作。
