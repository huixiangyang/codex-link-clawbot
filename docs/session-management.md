...
单独发送 `/` 打开视觉主菜单，再用数字进入会话中心。主菜单、会话列表、详情、状态和确认结果由固定 HTML 模板渲染为手机纵向 PNG；需要选择或输入时同时发送一条简短文字提示。会话中心展示当前会话、可用数量和归档数量，并提供详情、切换、新建、重命名、归档和恢复。当前会话详情继续提供重命名、切换和归档快捷入口；名称输入、归档确认和任务取消确认复用同一个两分钟短期状态机。回复 `0` 返回，回复“取消”退出当前操作。
