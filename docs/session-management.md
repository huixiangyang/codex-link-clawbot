...
单独发送 `/` 打开视觉主菜单，再用数字进入会话菜单。主菜单、会话列表、详情、状态和确认结果由固定 HTML 模板渲染为手机纵向 PNG；需要选择或输入时同时发送一条简短文字提示。会话菜单提供当前详情、切换、新建、重命名、归档和恢复；名称输入和归档确认也复用同一个两分钟短期状态机。回复 `0` 返回，回复“取消”退出当前操作。

同一能力也接受高置信度中文意图，例如“新建会话 叫登录排障”“切换会话 登录”“重命名当前会话 为发布检查”和“归档当前会话”。控制层只拦截完整意图；选择菜单时收到非数字普通文字会清理菜单状态并继续路由，不会静默吞消息。

切换和恢复支持服务端补全：先按名称、末尾短编号和完整 thread ID 进行精确匹配，再依次尝试前缀、包含和字符顺序匹配。唯一候选直接执行，多候选按匹配质量和最近活动顺序生成最多 8 个数字选项。所有候选都先经过本地所有权索引过滤。
