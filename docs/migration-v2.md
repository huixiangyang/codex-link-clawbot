...
- 可选配置 `security.remote_lock_code`、`voice.ffmpeg_command`、`voice.silk_command` 和 `voice.providers` 有序 TTS 提供商链。FFmpeg 与独立 SILK 编码器只属于共享发送层，部署时必须同时安装 `weclaw` 和 `weclaw-silk-encoder`；提供商级 `piper.ffmpeg_command`、旧扁平 MiMo 字段与 `voice.command` 均已删除。
