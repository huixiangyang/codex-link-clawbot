...
```bash
go build -ldflags '-X github.com/huixiangyang/weclaw/cmd.Version=v2.5.0-local.1' -o /absolute/path/to/weclaw .
...
当前生产会一次性跨越到包含 v2.6 与 v2.7 schema 的主线，不建立未验收的线上 v2.6 中间态。冻结、排空、备份、迁移、真机矩阵、回滚演练和稳定观察见 [v2.7 首次生产切换与真机验收](release-v2.7-cutover.md)。
